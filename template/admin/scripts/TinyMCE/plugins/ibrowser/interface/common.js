...
// ================================================
	...
	function iBrowser_open() {
		...
		} else { // Gecko or WebKit
		
			var height = 530;
			var width = 670;
			var left = (screen.width/2) - (height/2);
			var top = (screen.height/2) - (width/2);
			var wnd = window.open(ib.baseURL + '?editor=' + ib.editor + '&callback=iBrowser_callback', 'ibrowser', 'status=no, modal=yes, width='+width+', height='+height+', left='+left+', top='+top);
			wnd.moveTo(left, top);
