...
class ImageResize {
	...
	// resize options
	const OPT_RESIZE_INCREASE   = 1;
	const OPT_RESIZE_DECREASE   = 2;
	const OPT_RESIZE_ALWAYS     = 4;
	 ...
	 * @param string $method How to be converted
	 *     (auto, portrait, landspace, crop or exact)
	 * @param integer $resizeOn Resize on condition - always, only if new dimensions decrease or increase original
	 * @param mixed $color Color for background (HTML #rrggbb, css rgba() or array)
	 */
	public function resize($newWidth, $newHeight, $method = 'auto', $resizeOn = self::OPT_RESIZE_ALWAYS, $color = null)
	{

		$this->_resizeOn = $resizeOn;

		list($width, $height) = $this->_getDimensions($newWidth, $newHeight, $method);
	 ...
	 * @param integer $method How to be calucalate new sizes:
	 ...
	 */
	private function _getDimensions($newWidth, $newHeight, $method)
	{

		if ($this->_resizeOn & ~self::OPT_RESIZE_ALWAYS) {
			if ($this->_width > $newWidth && $this->_height > $newHeight && ($this->_resizeOn & self::OPT_RESIZE_DECREASE)) {
				return array($this->_width, $this->_height);
			}
			else if ($this->_width < $newWidth && $this->_height < $newHeight && ($this->_resizeOn & self::OPT_RESIZE_INCREASE)) {
				return array($this->_width, $this->_height);
			}
		}
