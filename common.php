...
// setup uploads file access mask
umask(0022);
...
// Init and return all available languages
if (SITE == 'client') {
	$i18n = InitLanguages();
	require(CONFIG.'messages.php');
} else {
...
$tmpl['locale.img'] = $tmpl['url'].'images'.DS.'locales'.DS.CFG('language').DS;
...
$skin->assign('FACEBOOK_LIKE', CFG('facebook.like.url'));
...
$skin->assign('IMG_LANG', $tmpl['locale.img']);
...
$skin->assign('VERSION', CFG('cache.version'));
$skin->assign('MAX_UPLOAD_COUNT', CFG('upload.max.count'));

// regional settings
$skin->assign('LANGS', $i18n);
$skin->assign('LANG', CFG('language'));
$skin->assign('LOCALE', CFG('locale'));
$skin->registerPlugin('block', 'tr', 'TranslateSmarty');
